    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md bg-slate-900 rounded-3xl p-8 shadow-2xl border border-white/10 text-slate-100">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold">歡迎回到日更の花圃</h1>
          <p className="text-white/50 text-sm mt-1">登入繼續你的日更之旅</p>
        </div>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-white/40 text-xs">或使用 Email 登入</span>
          <div className="flex-1 h-px bg-white/10"></div>
        </div>
        ...
        <form onSubmit={handleCredentials} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-brand-purple to-indigo-500 text-white font-bold py-4 rounded-2xl shadow-lg transition active:scale-95 disabled:opacity-50"
          >
            {loading ? "登入中..." : "登入"}
          </button>
        </form>
        ...
        <p className="text-center text-white/40 text-sm mt-6">
          還沒有帳號？{" "}
          <Link href="/auth/register" className="text-brand-purple hover:underline font-semibold">
            立即註冊
          </Link>
        </p>
      </div>
